#' ---
#' title: "Functions for analysis of CEO Data using NPIP Estimators"
...
#' ## Support Functions  
#' In order to use the esimators, the point data must be converted into a 
#' binary 'indicator' **y** for each cover class. 
...
calcError <- function(table1, table2, val1, err1, val2, err2) {
  # 
  new_values <- table1[,val1] * table2[,val2]
  names(new_values) <- "Value"
  
  new_errors <- new_values * sqrt((table1[,err1]/table1[,val1])^2 + 
                                    (table2[,err2]/table2[,val2])^2)
  names(new_errors) <- "Error"
  
  new_table <- bind_cols(table1[, -c(val1, err1)], new_values, new_errors)
  return(new_table)
}
...
#' All work based on the equations presented in __Patterson, P. L. (2012). 
#' Photo-based estimators for the Nevada photo-based inventory 
#' (Research Paper No. RMRS-RP-92). Fort Collins, CO. 
#' https://doi.org/10.2737/RMRS-RP-92__  
#' 
#' Equations numbers listed with functions indicate the equation in the above 
#' paper.  
#'
#' ### Proportion of cover.  
#' In the following equations:  
#' **p** = proportion of cover  
#' **P** = overall proportion of cover
#' **c** = a particular cover type  
#' **y_sub_c** = indicator variable (0,1) for presence of cover type *c*  
...
#' 
#' p_sub_ch[i] = sum(y_sub_chi[j])/m_sub_h[i]  
...
#' 
#' p_bar_sub_c[h] = sum(p_sub_ch[i])/n[h]  
...
#' P_hat_sub_c = sum(W[h] * P_bar_sub_c[h])  
...
#' V_hat(P_hat_sub_c)= sum(W[h]^2/(n[h](n[h]-1)) * 
#'  (sum(p_sub_ch[i]^2) - (sum(p_sub_ch[i])^2/n[h]))
...
overall_SE <- function(plotSummary, strata, areas, ns = NULL) {
  ...
  for (h in strata) {
    stratum <- subset(plotSummary, PL_StratumAll == sort(unique(PL_StratumAll))[h])[,-c(1:3)]
...
#' ## Analysis tools  
#' ### y_c Analysis tools  
#' A function to take the raw table, produce question tables for each question,
#'  and analyze them to produce the appropriate summaries. 
...
#' Functions for calculating y_o|c, an "object occuring in cover" conditional 
...
cond_SE <- function(yoctable, yctable, strata, grplst, conditions, covers,
  ...
  for (h in strata) {
    stratum_c <- subset(p_cSum, PL_StratumAll == h)[,-c(1:3)]
    stratum_oc <- subset(p_ocSum, PL_StratumAll == h)[,-c(1:3)]
...
do_yoc_analysis <- function(.data, mtdt, qstns, grplst, strata, ns, areas, 
  ...
  condTable <- build_question(.data, mtdt = metaNames, qstn = cndtnfld)
  coverTable <- build_question(.data, mtdt = metaNames, qstn = cvrfld)
  ...
  # build y_c table
  y_cTable <- build_yc(condTable, lblfld = cndtnfld, cmmdtylst = conditions)
  
  # build y_oc table
  y_ocTable <- build_yoc(table = .data, 
                          mtdt = metaNames, qstns = questions, grplst = groupList, 
                          cvrfld = cvrfld, cndtnfld = cndtnfld, 
  ...
  # gather results and make an object that can be converted into a pretty
  # cross table
  label <- str_split(names(p_hat_o_in_c), "-", 3, simplify = T)
  output <- tibble(label[,1], label[,3], p_hat_o_in_c, se_p_hat_o_in_c)
  colnames(output) <-  c("Cover", "Condition", "PercentCover", "SE")
  rownames(output) <- c()
