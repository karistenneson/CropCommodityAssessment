...
###########################
## remove non-crops
dataAll$source <- substr(dataAll$X,1,5)
unique(dataAll$source)

###########################
## remove non crops
dataAll<-dataAll[dataAll$source != 'NotFo',]
dataAll<-dataAll[dataAll$source != 'None_',]
unique(dataAll$source)
...
dataAll$sourceLabels <- recode(dataAll$source, 
                            "Aquacul" = paste0('Aquaculture', ' (', dataAll$Source_Total, ')'), 
                            "Palm_cr" = paste0('Palm crops', ' (', dataAll$Source_Total, ')'), 
                            "Coconut" = paste0('Coconut', ' (', dataAll$Source_Total, ')'), 
                            "Oil_pal" = paste0('Oil palm', ' (', dataAll$Source_Total, ')'), 
                            "RainfPt" = paste0('Tropical rainforest, peatlands', ' (', dataAll$Source_Total, ')'), 
                            "TrMouPt" = paste0('Tropical mountain system, peatlands', ' (', dataAll$Source_Total, ')'),
                            
                            "Coff" = paste0('Coffee', ' (', dataAll$Source_Total, ')'), 
                            "Crop" = paste0('Crop support', ' (', dataAll$Source_Total, ')'), 
                            "Frui" = paste0('Fruit & nut orchards', ' (', dataAll$Source_Total, ')'), 
...
###########################
## create target
dataAll$target[dataAll$level == 'Crop'] <- substr(dataAll$X[dataAll$level == 'Crop'],7,24)
...
dataAll$target[dataAll$level == 'Agro'] <- recode(dataAll$target[dataAll$level == 'Agro'],
                                            ...
                                            "Rubb_Agrisil" = 'Rubber, monoculture',
                                            "Rubb_Bndry" = 'Rubber, monoculture',
...
#                                            "Shrub_Trad" = 'Shrub crops, monoculture',
                                            "Tea_Agrisil" = 'Shrub crops, agrisilviculture',
                                            "Tea_Bndry" = 'Shrub crops, boundary tree',
                                            "Tea_Trad" = 'Shrub crops, monoculture',
...
TargetTotalsAll$LinkGroup[substr(TargetTotalsAll$name,1,3) %in% c('Coc',"Oil", "Pal")]<-'Palm'
...
write.csv(dataAll[,c('sourceLabels','targetLabels','Area_ha', 'SE_Area_ha')],paste0('Results/Fig3_Final/Fig3_', country, '_Formatted_Table.csv'), row.names=F)
...
sankeyPlot<- sankeyNetwork(Links = dataAll, Nodes = nodesAll, Source = "IDsource", Target = "IDtarget", Value = "Area_ha", 
                           ...
                           LinkGroup = "LinkGroup", 
                           NodeGroup="group", fontSize = 12, nodeWidth = 20,fontFamily = "Arial")
sankeyPlot

pdf("sankey/Fig3Indonesia.pdf")
# 2. Create the plot
sankeyNetwork(Links = dataAll, Nodes = nodesAll, Source = "IDsource", Target = "IDtarget", Value = "Area_ha", 
              NodeID = "name", colourScale=my_colorNoLink,
              iterations = 0, LinkGroup = "LinkGroup", 
