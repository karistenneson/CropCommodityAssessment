...
subset<-rawData[(rawData$PL_COUNTRY == 'Philippines' |rawData$PL_COUNTRY == 'Indonesia') & 
                  rawData$PL_STRATUM == 3
                  & rawData$LAND_USE != 'Natural_Forest' &  rawData$LAND_USE_Y == 'Natural_Forest',]
head(subset)
subset$LAND_COVER<-subset$UNDERSTORY<-subset$LAND_USE<-subset$LAND_USE_Y<-'ERROR'

rawData[(rawData$PL_COUNTRY == 'Philippines' |rawData$PL_COUNTRY == 'Indonesia') & 
          rawData$PL_STRATUM == 3 & rawData$LAND_USE != 'Natural_Forest' &  
          rawData$LAND_USE_Y == 'Natural_Forest',] <- subset
############################################################################
############################################################################
############################################################################
...
## Export Data
head(rawData)
write.csv(rawData, 'data\\EcoFloristicJoin\\Compiled_Processed_withSpatial_03122020Temp_INDO_PHIL_Errors.csv', row.names = F)
