...
################################################################################################
################################################################################################
# Add in unique Carbon classes
rawDataFull$Crop_Agro<-as.character(rawDataFull$Crop_Agro)
unique(rawDataFull$Crop_Agro)

rawDataFull$EcofloristicAbb<-as.character(rawDataFull$EcofloristicAbb)
unique(rawDataFull$EcofloristicAbb)

rawDataFull<-unite(rawDataFull, 'Forest_CropSys', c("EcofloristicAbb","Crop_Agro"), remove = FALSE)
unique(rawDataFull$Forest_CropSys)
rawDataFull$Forest_CropSys<-as.factor(rawDataFull$Forest_CropSys)

head(rawDataFull)

unique(rawDataFull$Crop_Agro)
unique(substr(rawDataFull$Crop_Agro,6,11))
rawDataFull$Crop_Agro2<-rawDataFull$Crop_Agro
rawDataFull$Crop_Agro2[substr(rawDataFull$Crop_Agro,6,11) == 'ndry']<-
  paste0(substr(rawDataFull$Crop_Agro[substr(rawDataFull$Crop_Agro,6,11) == 'ndry'],1,4), 'Agrisil')
rawDataFull$Crop_Agro2[substr(rawDataFull$Crop_Agro,6,11) == 'Bndry']<-
  paste0(substr(rawDataFull$Crop_Agro[substr(rawDataFull$Crop_Agro,6,11) == 'Bndry'],1,5), 'Agrisil')
sort(unique(rawDataFull$Crop_Agro2))
...
# 6. Thailand
stratumAreasThailand <- c("Strata1 Area" = 178405, "Strata2 Area" = 1334277, "Strata3 Area" = 49901653)		
# 7. Vietnam
stratumAreasVietnam <- c("Strata1 Area" = 348394, "Strata2 Area" = 1319799, "Strata3 Area" = 31059507)
...
# 6. Thailand
sampSizeThailand <- c(length(unique(rawDataFull[which(rawDataFull$PL_STRATUM == 1 & rawDataFull$PL_COUNTRY == "Thailand"),]$PL_PLOTID)),
...
# 7. Vietnam
sampSizeVietnam <- c(length(unique(rawDataFull[which(rawDataFull$PL_STRATUM == 1 & rawDataFull$PL_COUNTRY == "Vietnam"),]$PL_PLOTID)),
...
######################################################################################################################
######################################################################################################################
## statistics for the Carbon Difference Table.

unique(rawDataFull$PL_COUNTRY)
country<-"Vietnam"
stratumAreas <- stratumAreasVietnam
sampSize <- sampSizeVietnam

# subset the data by the country of interest.
rawData <- rawDataFull[rawDataFull$PL_COUNTRY == country,]
rawData$Forest_CropSys<-as.character(rawData$Forest_CropSys)
rawData$Forest_CropSys<-as.factor(rawData$Crop_Agro2)
...
questions <-  c("LAND_COVER", "UNDERSTORY", "LAND_USE", "LAND_USE_1", 
                ...
                'Crop_Agro', 'AgroSystem', 'EcoZne_Crop', 'Forest_Conv', 'Forest_CropSys')

qstn <- headerOfInterest<-'Crop_Agro2'
...
coverArea
write.csv(coverArea, paste0('Results/Table_Crops/Table_Crop_', country, 'ForesttoCropSys.csv'))
